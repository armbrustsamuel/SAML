...
def train(model, saver, sess, train_file_list, test_file, args, resume_itr=0):
    ...
    with tf.device('/cpu:0'):
        ...
        for i in range(len(train_file_list)):
            ...
            train_iterator_list.append(tf.data.Iterator.from_structure(
    ...
    for i in range(len(train_file_list)):
        training_init_op.append(
            train_iterator_list[i].make_initializer(tr_data_list[i].data))
    ...
    for epoch in xrange(0, args.epoch):
        for itr in range(resume_itr, args.train_iterations):
            ...
            input_group = np.concatenate(
                (inputa[:2], inputa1[:1], inputb[:2]), axis=0)
            label_group = np.concatenate(
                (labela[:2], labela1[:1], labelb[:2]), axis=0)
            ...
            output_tensors = [model.task_train_op,
                              model.meta_train_op, model.metric_train_op]
            output_tensors.extend([model.summ_op, model.seg_loss_b, model.compactness_loss_b,
            ...
            if itr % args.print_interval == 0:
                logging.info("Epoch: [%2d] [%6d/%6d] time: %4.4f inner lr:%.8f outer lr:%.8f" % (
                    epoch, itr, args.train_iterations, (time.time()-start), model.inner_lr.eval(), model.outer_lr.eval()))
                logging.info('sou_loss: %.7f, tar_loss: %.7f, tar_seg_loss: %.7f, tar_compactness_loss: %.7f, tar_smoothness_loss: %.7f' % (
                    source_loss, target_loss, seg_loss_b, compactness_loss_b, smoothness_loss_b))
            ...
            if (itr != 0) and itr % args.save_freq == 0:
                saver.save(sess, args.checkpoint_dir + '/epoch_' +
                           str(epoch) + '_itr_'+str(itr) + ".model.cpkt")
...
def test(sess, test_list, model, args):
    ...
    for fid, filename in enumerate(test_list):
        ...
        for ii in xrange(int(np.floor(image.shape[2] // model.test_batch_size))):
            vol = np.zeros([model.test_batch_size, model.volume_size[0],
                           model.volume_size[1], model.volume_size[2]])
...
def _save_nii_prediction(gth, comp_pred, pre_pred, out_folder, out_bname):
    sitk.WriteImage(sitk.GetImageFromArray(
        gth), out_folder + out_bname + 'gth.nii.gz')
    sitk.WriteImage(sitk.GetImageFromArray(pre_pred),
                    out_folder + out_bname + 'premask.nii.gz')
    sitk.WriteImage(sitk.GetImageFromArray(comp_pred),
                    out_folder + out_bname + 'mask.nii.gz')
